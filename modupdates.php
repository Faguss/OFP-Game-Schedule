...
if ($lang["THIS_CODE"] == "en-US") {
	$lang = array_merge($lang, array(
		...
		#Add a new version
		"GS_MU_SECTION1_PAR1" => "On the OFP Game Schedule website you can indicate that you've updated your mod and then players will be able to download the new files.",
		...
		"GS_MU_SECTION1_PAR8" => " <a href=\"install_scripts\" target=\"_blank\">Installer documentation</a>",
		...
		"GS_MU_SECTION2_PAR3" => "You can see what the installation process will look like by clicking on the \"Preview Installation\" button on the bottom of the \"Installation\" page.",
		"GS_MU_SECTION2_PAR4" => "Every time this mod gets updated the users will redownload the same file. This might get burdensome with large archives so instead you could provide a new, smaller package that only contains the patched files (option #2).",
		...
		"GS_MU_SECTION3_PAR2" => "You can reuse script from any other version by selecting it from the \"Installation script\" list. Alternatively pick \"Add a new script\" to create a new one for this version.",
		...
		"GS_MU_SECTION4_PAR1" => "Jumps can be used to skip updates that became obosolete.",
		"GS_MU_SECTION4_PAR2" => "Look again at the example with multiple scripts (option #2).",
		"GS_MU_SECTION4_PAR3" => "Let's assume that <code>patch2.zip</code> already contains all the changes from the <code>patch1.zip</code> so the latter is unnecessary. We could skip it so that players will download less data.",
		"GS_MU_SECTION4_PAR4" => "Go to the \"Installation\" page and select \"Jumps Between Versions\". In the field \"From version\" type \"version = 1\" to target users with the first version of the mod. Select below \"Installation script: Same as in version 1.1 to 1.2\" which downloads <code>patch2.zip</code>. Finally click on the \"Add New Jump\".",
		...
		"GS_MU_SECTION4_PAR7" => "In the field \"From version\" write \"version = 0\" to target new users. From the list \"To version\" select \"Always to the newest one\". Write the new installation script below and then click on the \"Add New Jump\".",
		"GS_MU_SECTION4_PAR8" => "Now every time you add a new version this jump will be automatically adjusted. Don't forget to upload the new <code>mod_new.zip</code>.",
		
		#Testing
		"GS_MU_SECTION5_PAR1" => "Create a new mod just for testing. Add a password to hide it from other users.",
		"GS_MU_SECTION5_PAR2" => "Now you can add new versions and jumps to find out how they function.",
...
if ($lang["THIS_CODE"] == "pl-PL") {
	$lang = array_merge($lang, array(
		...
		#Add a new version
		"GS_MU_SECTION1_PAR1" => "Na stronie Rozkładu Rozgrywek do OFP możesz zaznaczyć, że dokonałeś zmian w modzie, a następnie gracze będą mogli pobrać uaktualnienie.",
		...
		"GS_MU_SECTION1_PAR8" => "<a href=\"install_scripts\" target=\"_blank\">Dokumentacja instalatora</a>",
		...
		"GS_MU_SECTION2_PAR3" => "Proces instalacyjny możesz podejrzeć klikając na przycisk \"Podgląd Instalacji\" na dole strony \"Instalacja\".",
		...
		"GS_MU_SECTION3_PAR2" => "Możesz ponownie wykorzystać skrypt z dowolnej innej wersji wybierając go z listy \"Skrypt instalacyjny\". Opcjonalnie wybierz opcję \"Dodaj nowy skrypt\" aby zamienić obecny skrypt na nowy.",
		...
		#Jumps between versions
		"GS_MU_SECTION4_PAR1" => "Skoki służą do pomijania przestarzałych aktualizacji.",
		"GS_MU_SECTION4_PAR2" => "Wrócmy do przykładu z wieloma skryptami instalacyjnymi (wariant #2).",
		"GS_MU_SECTION4_PAR3" => "Załóżmy, że archiwum <code>patch2.zip</code> zawiera już wszystkie zmiany z <code>patch1.zip</code> i to ostatnie jest zbędne. Możemy je pominąć, żeby gracze mieli mniej danych do ściągania.",
		"GS_MU_SECTION4_PAR4" => "Przejdź do strony \"Instalacja\". Wybierz \"Skoki pomiędzy wersjami\" na górze strony. W polu \"Z wersji\" wpisz \"version = 1\" żeby zaadresować użytkowników z pierwszą wersją moda. Poniżej wybierz \"Skrypt instalacyjny: Taki sam jak w wersji 1.1 do 1.2\" który ściąga <code>patch2.zip</code>. Na końcu wciśnij \"Dodaj nowy skok\".",
		...
		"GS_MU_SECTION4_PAR8" => "Od teraz, za każdym razem gdy dodasz nową wersję ten skok zostanie automatycznie dopasowany. Nie zapomnij uaktualnić pliku <code>mod_new.zip</code>.",
		
		#Testing
		"GS_MU_SECTION5_PAR1" => "Utwórz nowy mod wyłącznie do testowania. Dodaj hasło, żeby nie pokazywał się innym użytkownikom.",
		"GS_MU_SECTION5_PAR2" => "Teraz możesz dodawać nowe wersje i skoki aby móc sprawdzać jak działają.",
...
#Add a new version
"GS_MU_SECTION1_PAR1" => "На веб-сайте расписания игр OFP вы можете указать, что вы обновили свой мод, и тогда игроки смогут загрузить новые файлы.",
...
"GS_MU_SECTION1_PAR8" => "<a href=\"install_scripts\" target=\"_blank\">Документация для установщика</a>",
...
"GS_MU_SECTION2_PAR3" => "Посмотреть, как будет выглядеть процесс установки, можно, нажав кнопку «Предварительный просмотр установки» внизу страницы «Установка».",
...
"GS_MU_SECTION3_PAR2" => "Вы можете повторно использовать скрипт из любой другой версии, выбрав его из списка «Установочный скрипт». Альтернативно выберите «Добавить новый скрипт», чтобы создать новый для этой версии.",
...
#Jumps between versions
"GS_MU_SECTION4_PAR1" => "Переходы можно использовать для пропуска обновлений, которые устарели.",
"GS_MU_SECTION4_PAR2" => "Посмотрите еще раз на пример с несколькими скриптами (вариант №2).",
"GS_MU_SECTION4_PAR3" => "Предположим, что <code>patch2.zip</code> уже содержит все изменения из <code>patch1.zip</code>, поэтому последний не нужен. Мы могли бы пропустить это, чтобы игроки скачивали меньше данных.",
"GS_MU_SECTION4_PAR4" => "Перейдите на страницу «Установка» и выберите «Переход между версиями». В поле «Из версии» введите «версия = 1», чтобы настроить таргетинг на пользователей с первой версией мода. Ниже выберите «Сценарий установки: такой же, как в версиях с 1.1 по 1.2», который загружает <code>patch2.zip</code>. Наконец нажмите «Добавить новый переход».",
...
"GS_MU_SECTION4_PAR7" => "В поле «Из версии» напишите «версия = 0», чтобы настроить таргетинг на новых пользователей. Из списка «К версии» выберите «Всегда к самой новой версии». Напишите новый сценарий установки ниже и нажмите «Добавить новый переход».",
"GS_MU_SECTION4_PAR8" => "Теперь каждый раз, когда вы добавляете новую версию, этот переход будет автоматически корректироваться. Не забудьте загрузить новый <code>mod_new.zip</code>.",
		
#Testing
"GS_MU_SECTION5_PAR1" => "Создайте новый мод только для тестирования. Добавьте пароль, чтобы скрыть его от других пользователей.",
"GS_MU_SECTION5_PAR2" => "Теперь вы можете добавлять новые версии и переходы, чтобы узнать, как они работают.",
